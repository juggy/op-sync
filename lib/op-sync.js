...
OS.SyncLog = (function() {
  ...
  SyncLog.prototype.sync_backlog = function() {
    ...
    success = __bind(function(incoming) {
      this.add_to_backlog(this.parse_incoming(incoming));
    ...
    if (this.device.debug) {
      _log("Sync Backlog", this.get_backlog());
    }
    params = {
      success: success,
      error: error,
  ...
  SyncLog.prototype.reset_synced = function() {
    var backlog_local, index, last, spliced, tr, _i, _len, _results;
    backlog_local = _.clone(this.get_backlog());
    index = 0;
    spliced = 0;
    last = "";
    _results = [];
  ...
  SyncLog.prototype.post_sync_process = function(incoming) {
    ...
    if (this.device.debug) {
      _log("Post Sync: ", this.get_backlog());
    }
    this.reset_synced();
    if (this.device.debug) {
      _log("Post Sync after reset: ", this.get_backlog());
    }
    _ref2 = this.instance_index();
    ...
    this.reset_synced();
    if (this.device.debug) {
      return _log("Post Sync after reset 2: ", this.get_backlog());
    }
  };
  SyncLog.prototype.get_backlog = function() {
    if (!this.sorted) {
      this.sorted = true;
      this.backlog = _.sortBy(this.backlog, function(i) {
        return i.timestamp;
      });
    }
    return this.backlog;
  ...
  SyncLog.prototype.reset_index = function() {
    this.sorted = false;
    return this._instance_index = null;
  };
